// mini_redis.cpp
// Combined: custom HashTable (chaining) + CLI + deque history
// Compile: g++ -std=c++17 -O2 -o mini_redis mini_redis.cpp
// Run: ./mini_redis
...
// ------------------ Simple HashTable (chaining) ------------------
// Avoids using unordered_map; custom implementation for coursework
struct Entry {
    string key;
    string value;
...
public:
    ...
    void set(const string &key, const string &value) {
        ...
        ++count;
        if ((double)count / buckets.size() > load_factor_threshold) rehash();
    }
    ...
    bool del(const string &key) {
        ...
        for (size_t i = 0; i < bucket.size(); ++i) {
            if (bucket[i].key == key) {
                // remove by swapping with last for O(1) remove (order in bucket not important)
    ...
    // return all keys (for KEYS command)
...
private:
    ...
    // simple polynomial rolling hash -> index
    unsigned long long str_hash(const string &s) const {
        const unsigned long long p = 1315423911ULL; // arbitrary multiplier
...
int main() {
    ...
    while (true) {
        ...
        if (history_deque.size() > HISTORY_MAX) history_deque.pop_front();

        // Tokenize and uppercase command
        ...
        if (CMD == "EXIT") {
            ...
            string key = parts[1];
            // join remaining parts into value? original expects single token; keep single token for simplicity:
            ...
            if (ht.get(key, out)) cout << out << endl;
            else cout << "(nil)" << endl;
        }
        else if (CMD == "DEL") {
            if (parts.size() != 2) {
            ...
            if (n > (int)history_deque.size()) n = (int)history_deque.size();
